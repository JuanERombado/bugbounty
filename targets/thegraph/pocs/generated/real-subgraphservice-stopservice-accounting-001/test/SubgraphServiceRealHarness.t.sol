...
contract SubgraphServiceRealHarnessTest is Test {
    ...
    // Hypothesis: STOPSERVICE-ACCOUNTING-001
    // Local-only invariant: stopService should close the allocation and release aggregate accounting.
    function test_STOPSERVICE_ACCOUNTING_001_stopServiceClosesAllocationAndReleasesAccounting() public {
    ...
    function test_STOPSERVICE_ACCOUNTING_001_cannotStopClosedAllocationTwice() public {
        vm.warp(100);
        vm.prank(indexer);
        service.stopService(indexer, abi.encode(allocationId));
    ...
    function test_STOPSERVICE_ACCOUNTING_001_unauthorizedStopServiceReverts() public {
        vm.prank(attacker);
        vm.expectRevert();
        service.stopService(indexer, abi.encode(allocationId));
    }
